...
//
// Guarded (lib/security/guard.ts): same-origin, session, per-user rate limit. The brief call is
// optional (answers save either way), so the AI budget is checked inside chatCompletion.
...
export async function POST(req: Request) {
    const g = await guardApi(req, RATE.about, { what: "questionnaire saves" });
    if (!g.ok) return g.response;
    const uid = g.uid;

    const parsed = await readJsonBody<{ answers?: unknown; followUps?: unknown; followUpsHash?: unknown; status?: unknown }>(req, 256 * 1024);
    ...
    return withAiUser(uid, "about", async () => {
        try {
            ...
            const next: Characterization = {
                ...
                answers,
                followUps,
                followUpsHash: typeof body.followUpsHash === "string" ? body.followUpsHash.slice(0, 64) : stored.followUpsHash,
                answersHash: hash,
                facts,
            };
