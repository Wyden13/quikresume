...
interface EditorItemRowProps {
    ...
    open: boolean;
    onToggleOpen: () => void;
    /** Scroll this row into view and focus its first input when it mounts (freshly added items). */
    autoFocus?: boolean;
    children: React.ReactNode;
}

export function EditorItemRow({ id, title, subtitle, meta, isSelected, onToggleSelected, onRemove, usedBy, stale, open, onToggleOpen, autoFocus, children }: EditorItemRowProps) {
    ...
    return (
        <ExpandableRow id={`edit-${id}`} open={open} onToggle={onToggleOpen} summary={summary} controls={controls} muted={!isSelected}>
